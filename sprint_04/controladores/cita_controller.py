...
class CitaController:
    ...
    def eliminar_cita_por_id(self, id_cita):
        """Elimina una cita permanentemente (no solo cancelar)."""
        citas = self.listar_todas()
        for i, cita in enumerate(citas):
            if cita.get("id_cita") == id_cita:
                citas.pop(i)
                JsonManager.guardar(self.ruta_db, citas)
                return True, f"Cita {id_cita} eliminada permanentemente."
        return False, f"No se encontró la cita con ID {id_cita}."

    def eliminar_todas_las_citas(self):
        """Elimina todas las citas del sistema (con backup)."""
        citas = self.listar_todas()
        if not citas:
        ...
        return True, f"Se eliminaron {cantidad} citas. Backup en: {backup['destino']}"
