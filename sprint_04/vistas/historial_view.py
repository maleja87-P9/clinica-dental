...
class HistorialView(QWidget):
    ...
    def _construir_ui(self):
        ...
        # Tabla con 6 columnas (incluyendo "Paciente")
        self.tabla = QTableWidget(0, 6)
        self.tabla.setHorizontalHeaderLabels(
            ["Fecha", "Paciente", "Odontólogo", "Diagnóstico", "Tratamiento", "Observaciones"]
        )
    ...
    def consultar_historial(self):
        ...
        if not paciente:
            ...
            return
        nombre_paciente = paciente.get("nombre", "Sin nombre")
        registros = self.historial_controller.consultar_por_paciente(documento)
        self.tabla.setRowCount(len(registros))
        for fila, registro in enumerate(registros):
            self.tabla.setItem(fila, 0, QTableWidgetItem(registro.get("fecha", "")))
            self.tabla.setItem(fila, 1, QTableWidgetItem(nombre_paciente))
            self.tabla.setItem(fila, 2, QTableWidgetItem(registro.get("odontologo", "")))
            self.tabla.setItem(fila, 3, QTableWidgetItem(registro.get("diagnostico", "")))
            self.tabla.setItem(fila, 4, QTableWidgetItem(registro.get("tratamiento", "")))
            self.tabla.setItem(fila, 5, QTableWidgetItem(registro.get("observaciones", "")))
