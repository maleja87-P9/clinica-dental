...
class NotificacionesView(QWidget):
    # El constructor acepta: notificacion_controller, paciente_controller (opcional), parent (opcional)
    def __init__(self, notificacion_controller, paciente_controller=None, parent=None):
    ...
    def _construir_ui(self):
        ...
        # Tabla con 7 columnas (agregamos "Nombre Paciente")
        self.tabla = QTableWidget(0, 7)
        self.tabla.setHorizontalHeaderLabels(
            ["ID", "Documento", "Nombre Paciente", "Fecha", "Tipo", "Mensaje", "Estado"]
        )
    ...
    def _obtener_nombre_paciente(self, documento):
        if self.paciente_controller:
            paciente = self.paciente_controller.buscar_por_documento(documento)
            return paciente.get("nombre", "Desconocido") if paciente else "Desconocido"
        return "No disponible"

    def registrar_notificacion(self):
        documento = self.input_documento.text().strip()
        if not documento:
            QMessageBox.warning(self, "Notificaciones", "El documento del paciente es obligatorio.")
            return
    ...
    def buscar_por_paciente(self):
        ...
        if not documento:
            ...
            return
        notificaciones = self.notificacion_controller.buscar_por_paciente(documento)
        self._cargar_tabla(notificaciones)
    ...
    def cargar_desde_tabla(self):
        ...
        self.input_mensaje.setPlainText(self.tabla.item(fila, 5).text())
        self.combo_tipo.setCurrentText(self.tabla.item(fila, 4).text())

    def _cargar_tabla(self, registros):
        self.tabla.setRowCount(len(registros))
        for fila, notificacion in enumerate(registros):
            documento = notificacion.get("documento_paciente", "")
            nombre = self._obtener_nombre_paciente(documento)
            self.tabla.setItem(fila, 0, QTableWidgetItem(notificacion.get("id_notificacion", "")))
            self.tabla.setItem(fila, 1, QTableWidgetItem(documento))
            self.tabla.setItem(fila, 2, QTableWidgetItem(nombre))
            self.tabla.setItem(fila, 3, QTableWidgetItem(notificacion.get("fecha_creacion", "")))
            self.tabla.setItem(fila, 4, QTableWidgetItem(notificacion.get("tipo", "")))
            self.tabla.setItem(fila, 5, QTableWidgetItem(notificacion.get("mensaje", "")))
            self.tabla.setItem(fila, 6, QTableWidgetItem(notificacion.get("estado", "")))
